...
import HistoryIcon from "@/assets/history (1).png"
...
export default function SearchBar() {
    ...
    const [searchSug, setSearchSug] = useState<{ value: string }[] | null>(dummydata)
    ...
    const callApi = async () => {
        ...
        try {
            // const response = await fetch(`https://api.addsearch.com/v1/suggest/1bed1ffde465fddba2a53ad3ce69e6c2?term=${inputValue}`)
    ...
    return (
        <div>
            <div className={`relative rounded-3xl ${isFocused ? 'bg-[#303135]' : 'bg-[#4e5156]'}`}>
                <div className={`mx-4 flex justify-between py-3 ${(isFocused && searchSug) && 'border-b border-gray-400'}`}>
                    ...
                    <input
                        ...
                        className='bg-transparent w-[450px] font-bold px-2 outline-none'
                        value={inputValue}
                    ...
                    <span className='group px-1 cursor-pointer flex items-center relative'>
                        ...
                        <span className='opacity-0 group-hover:opacity-100 transition-opacity absolute top-10 -right-10 bg-darkGray border-hGray border-1 w-28 text-[12px] font-bold py-1 flex justify-center'>
                            Search by voice
                        </span>
                    </span>
                    <span className='group px-1 cursor-pointer flex items-center relative'>
